...
class WXOResource:
    ...
    def _get_required_element_from_dict(
        ...
        WXOResource._validate_type(el, root_path, dict)
        WXOResource._validate_type(root_path, "root_path", str)
        WXOResource._validate_type(path, "path", list)

        if path.__len__() < 1:
            raise ClientError("Unexpected path length: {}".format(path.__len__))
        ...
        if path.__len__() > 1:
            return WXOResource._get_required_element_from_dict(
    ...
    def _get_asset_based_resource(
        ...
        # WXOResource._validate_type(asset_id, f"{asset_type}_id", str, False)
        ...
        if asset_id:
            ...
            if not get_required_element_from_response:
                return response
    ...
    def _get_with_or_without_limit(
        ...
        if limit is not None:
            if limit < 1:
                raise ClientError("Limit cannot be lower than 1.")
            elif limit > 200:
                raise ClientError("Limit cannot be larger than 200.")
