...
class ServiceInstance:
        ...
        **Example**
        ...
        if self._client.ICP_PLATFORM_SPACES:
            if self._credentials.username is not None:
                ...
                raise ClientError("`username` missing in credentials.")
        else:
            raise ClientError("Not applicable for Cloud")
        ...
        **Example**
        ...
        if self._client.ICP_PLATFORM_SPACES:
            if self._credentials.password is not None:
                ...
                raise ClientError("`password` missing in credentials.")
        else:
            raise ClientError("Not applicable for Cloud")
    ...
    def _create_token(self) -> str:
        ...
        if self._client.CLOUD_PLATFORM_SPACES:
            if self._client._is_MCSP():
                return self._get_MCSP_token()
            else:
                raise ClientError(
    ...
    def _get_IAM_token(self) -> str:
        ...
        if response.status_code == 200:
            ...
            raise ClientError("Error getting IAM Token.", response)
        return token

    def _get_MCSP_token(self) -> str:
        if self._client.proceed is True:
            return self._credentials.token
        headers = {
        ...
        response = requests.post(
            self._href_definitions.get_mcsp_token_url(), json=payload, headers=headers
        )
